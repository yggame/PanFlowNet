...
path_ms = "/gdata2/yanggang/datasets/yaogan/WV2_data/test128/ms"
path_pan = "/gdata2/yanggang/datasets/yaogan/WV2_data/test128/pan"
#path_predict = "/ghome/fuxy/DPFN-master-GF2/results/pnn_GF2/test"
path_predict = "/ghome/yanggang/yg_code/pan_inn/results/gppnn_WV2/test_16stage/07-24-12-52/"

def cal_performance(path_ms, path_pan, path_predict=path_predict, log_name=None):
    print("The path of predict:", path_predict)
    save_config(log_name, "The path of predict: {}".format(path_predict))
    message = ""
    list_name = []
    ...
    for file_name_i in list_name:
        ...
        if fnb == num:
            ...
            message +='################## reference comparision ####################### \n'
            for index, i in enumerate(ref_results):
                if index == 0:
                    print(i, ref_results[i])
                    message += str(i) + str(ref_results[i]) + "\n"
                else:
                    print(i, [round(j, 4) for j in ref_results[i]])
                    message += str(i) + str([round(j, 4) for j in ref_results[i]]) + "\n"
            print('################## reference comparision #######################')
            message += '################## reference comparision ####################### \n'


            print('################## no reference comparision ####################')
            message += '################## no reference comparision #################### \n'
            for index, i in enumerate(no_ref_results):
                if index == 0:
                    print(i, no_ref_results[i])
                    message += str(i) + str(no_ref_results[i]) + "\n"
                else:
                    print(i, [round(j, 4) for j in no_ref_results[i]])
                    message += str(i) + str([round(j, 4) for j in no_ref_results[i]]) + "\n"
            print('################## no reference comparision ####################')
            message += '################## no reference comparision #################### \n'
            save_config(log_name, message)

    return ref_results,no_ref_results
