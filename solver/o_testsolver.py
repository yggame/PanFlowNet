...
class Testsolver(BaseSolver):
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            self.save_img(bms_image.cpu().data, name[0][0:-4]+'_bic.tif', mode='CMYK')  # CMYK
            self.save_img(ms_image.cpu().data, name[0][0:-4]+'_gt.tif', mode='CMYK')
            self.save_img(prediction.cpu().data, name[0][0:-4]+'.tif', mode='CMYK')
            self.save_img(noise.cpu().data, name[0][0:-4]+'_noise.tif', mode='CMYK')
        ...
        print('best_no_ref_results', self.best_no_ref_results['deep    '])
    ...
    def save_img(self, img, img_name, mode):
        ...
        #print(save_img.max())
        save_img = Image.fromarray(save_img[:,:,:4], mode)
